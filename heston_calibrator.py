...
from calibrator_utils import ConditionalPosteriorHestonMixin
...
class HestonCalibrator(BasicCalibrator, ConditionalPosteriorHestonMixin):
    ...
    def __init__(self, alpha_prior: float = 2.0, beta_prior: float = 0.005, p_prior: float = 2.0,
                 ...
                 kappa_prior: float = 0.0, sigma_sq_kappa_prior: float = 1.0, mu: float = 0.01,
                 kappa: float = 5, theta: float = 0.0225, omega: float = 0.02, psi: float = 0.0,
                 *args, **kwargs):
        ...
        self.alpha_star = self.steps / 2 + self.alpha_prior
    ...
    def calibrate(self, n_mcmc_steps, burn_in, rejection_rate=0.01):
        # ----- generate starting values for V using a truncated normal distribution
        #       (present-time as well as shifted backward and forward)
        V_t_array = np.array(truncnorm.rvs(a=0, b=np.inf, loc=0.0225, scale=0.005, size=self.steps + 2))
        ...
        mu_final = np.median(self._all_params_array_no_burnin[:, 0])
        kappa_final = np.median(self._all_params_array_no_burnin[:, 1])
        theta_final = np.mean(self._all_params_array_no_burnin[:, 2])
        psi_final = np.median(self._all_params_array_no_burnin[:, 3])
        omega_final = np.median(self._all_params_array_no_burnin[:, 4])
    ...
    def get_paths(self, T, s0, nsteps, nsim, risk_neutral=False):
        ...
        for i in range(nsteps):
            ...
            # simulated_volas[:, i + 1] = simulated_volas[:,i] + kappa*(theta - simulated_volas[:,i])*dt + sigma*np.sqrt(simulated_volas[:,i])*Z_V[:,i+1]
            ...
            # simulated_paths[:,i+1] = simulated_paths[:,i] + mu*simulated_paths[:,i]*dt + np.sqrt(simulated_volas[:,i])*simulated_paths[:,i]*np.sqrt(dt)*Z_S
